...
!
! Linux or Mac:
! gfortran -o G3R G3R_v2023.f95 -Ofast
!
! Windows:
! gfortran -o G3R.exe G3R_v2023.f95 -Ofast
!
! To run program use file CDR.txt with input parameters
!
! Linux or Mac:
! ./G3R < CDR.txt
!
...
PROGRAM main
    ...
    READ(5,*) ax2,ecc2,inc2,m2,ome2,peri2
    total_time = SECOND()
    m = (/mass1,mass2,mass3/)
    duration = SECOND()
    irev = itest (iwr,ns,ist,max_outer,m,ax1,ecc1,inc1,m1,ome1,peri1,ax2,ecc2,inc2,m2,ome2,peri2)
    if (irev >= max_outer) THEN
        WRITE (idebug, '("Stable in",I8," revolutions; duration: ",F7.2," s")') irev, SECOND() - duration
    END IF
    WRITE (idebug,'("Total time:",F7.2," s")') SECOND() - total_time
    CALL FLUSH(idebug)
END PROGRAM MAIN

FUNCTION itest (iwr,ns,ist,max_outer,m,ax1,ecc1,inc1,m1,ome1,peri1,ax2,ecc2,inc2,m2,ome2,peri2)
    IMPLICIT REAL*8 (a-h,m,o-z)
    COMMON /aaa/    a1, a2, ene0, prd1
    COMMON /diagno/ energy
    COMMON /jeet/   eej(3)
    ...
    DO
        ...
        END IF
        IF (is == ist) THEN
            WRITE(6,106)ytime/tmax/prd0*100.0,char(13)
106         FORMAT(F7.2,' % completed', A1,$)
            ...
            dene = enet/ene0 - 1
            IF(ene00 == 0D0) ene00 = enet
            elost = ene0/ene00 - 1 ! Relative difference of relativistic and newtonian energy (not error).
            ...
            IF (iwr > 0) THEN
                m12=m(1)+m(2)
                WRITE(66,123) ytime,(-m(2)/m12*xw(j),j=1,3),(m(1)/m12*xw(j),j=1,3),(xw(j),j=4,6)
            END IF
            cosi12=CDOT(ej1,ej2)/SQRT(CDOT(ej1,ej1)*CDOT(ej2,ej2))
            WRITE(91,191) ytime/prd0*twopi/period_ratio,ytime,a12,e12,COS(inc12*DEG),&
            a3,e3,COS(inc3*DEG), cosi12,&
            oo12, oo3, om12, om3, om3-om12
123    FORMAT(1X,1P,G12.2,9G12.4,' 0 0 0')
191    FORMAT(1P,G12.4,G12.4,G12.4,0P,F9.5,F7.2,1P,G12.4,0P,F9.5,7F12.5)
            IF((a12<0).or.(a3<0).or.(a12>a3).or.(e12>1).or.(e3>1)) THEN
!            IF((a12<0).or.(a3<0).or.(a12>a3).or.(a3>10*ax2)) THEN
                WRITE (idebug, '("Instabilty on", I8, "th period of outer binary")')&
                    INT(ytime/prd0*twopi/period_ratio)
                itest = INT(ytime/prd0*twopi/period_ratio)
                RETURN
            END IF
        END IF
...
SUBROUTINE si3(h, x, v, m, ttime, ifi)
    ...
    END DO
    IF (ite >= 100) THEN
        !WRITE(idebug, *),'No convergence'
        !WRITE(idebug, *),'test: ',test,' h: ',h
        iproblem = 1
        RETURN
    END IF
    y2van   = y2
...
SUBROUTINE cfun(z,c)
    ...
    IF (large < 10)  THEN
        large = large + 1
        !WRITE(idebug, *),' too large z in cfun!',z, large, char(7)
    END IF
2   CONTINUE
